...
import uk.gov.justice.digital.hmpps.electronicmonitoringdatastoreapi.resource.EquipmentDetailsController
...
class EquipmentDetailsControllerTest {
  ...
  inner class GetEquipmentDetails {
    ...
    fun `gets order information from order service`() {
      ...
      Mockito.`when`(equipmentDetailsService.getEquipmentDetails(orderId, false)).thenReturn(expectedResult)
      ...
      Mockito.verify(equipmentDetailsService, Mockito.times(1)).getEquipmentDetails(orderId, false)
    }
  }
}
