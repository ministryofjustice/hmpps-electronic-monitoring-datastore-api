...
import uk.gov.justice.digital.hmpps.electronicmonitoringdatastoreapi.resource.ViolationEventsController
...
@ActiveProfiles("test")
@JsonTest
class ViolationEventsControllerTest {
  ...
  fun setup() {
    ...
    controller = ViolationEventsController(orderEventsService, roleService, auditService)
  }
  ...
  inner class GetViolationEvents {
    ...
    fun `gets violation events from order events service`() {
      ...
      Mockito.`when`(orderEventsService.getViolationEvents(legacySubjectId, AthenaRole.ROLE_EM_DATASTORE_GENERAL_RO)).thenReturn(expectedResult)
      ...
      Mockito.verify(orderEventsService, Mockito.times(1)).getViolationEvents(legacySubjectId, AthenaRole.ROLE_EM_DATASTORE_GENERAL_RO)
    }
  }
}
