...
class AlcoholMonitoringVisitDetailsRepositoryTest {
  ...
  fun setup() {
    ...
    alcoholMonitoringVisitDetailsRepository = AlcoholMonitoringVisitDetailsRepository(emDatastoreClient)
  }
  ...
  inner class GetVisitDetails {
    ...
    fun `getVisitDetails passes correct query to getQueryResult`() {
      val resultSet = AthenaHelper.resultSetFromJson(amVisitDetailsResultSet("123"))

      Mockito.`when`(emDatastoreClient.getQueryResult(any<SqlQueryBuilder>(), eq(false))).thenReturn(resultSet)

      alcoholMonitoringVisitDetailsRepository.findByLegacySubjectId("123")
    ...
    fun `getVisitDetails returns all the results from getQueryResult`() {
      val resultSet = AthenaHelper.resultSetFromJson(amVisitDetailsResultSet("987"))

      Mockito.`when`(emDatastoreClient.getQueryResult(any<SqlQueryBuilder>(), eq(false))).thenReturn(resultSet)

      val result = alcoholMonitoringVisitDetailsRepository.findByLegacySubjectId("987")
