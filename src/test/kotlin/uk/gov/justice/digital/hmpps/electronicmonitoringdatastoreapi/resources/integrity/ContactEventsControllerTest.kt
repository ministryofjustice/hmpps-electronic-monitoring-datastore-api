...
class ContactEventsControllerTest {
  ...
  fun setup() {
    ...
    controller = ContactEventsController(orderEventsService, auditService)
  }
  ...
  inner class GetContactEvents {
    ...
    fun `gets contact events from order events service`() {
      ...
      `when`(orderEventsService.getContactEvents(legacySubjectId)).thenReturn(expectedResult)
      ...
      Mockito.verify(orderEventsService, Mockito.times(1)).getContactEvents(legacySubjectId)
    }
  }
}
