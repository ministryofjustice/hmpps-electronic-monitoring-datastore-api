...
import uk.gov.justice.digital.hmpps.electronicmonitoringdatastoreapi.resource.IncidentEventsController
...
class IncidentEventsControllerTest {
  ...
  inner class GetIncidentEvents {
    ...
    fun `gets incident events from order events service`() {
      ...
      `when`(orderEventsService.getIncidentEvents(legacySubjectId, AthenaRole.ROLE_EM_DATASTORE_GENERAL_RO)).thenReturn(expectedResult)
