...
class OrderControllerTest {
  ...
  @Nested
  inner class GetGeneralOrderSummary {
    @Test
  ...
  @Nested
  inner class GetRestrictedOrderSummary {
    @Test
    fun `gets restricted order information from order service`() {
      val orderId = "1ab"
      val expectedResult = OrderInformation(
        keyOrderInformation = KeyOrderInformation(
          specials = "NO",
      ...
      `when`(orderService.getOrderInformation(orderId, true)).thenReturn(expectedResult)

      val result = controller.getRestrictedOrderSummary(authentication, orderId)

      Assertions.assertThat(result.statusCode).isEqualTo(HttpStatus.OK)
      Assertions.assertThat(result.body).isNotNull
      Assertions.assertThat(result.body).isInstanceOf(OrderInformation::class.java)
      Assertions.assertThat(result.body).isEqualTo(expectedResult)
      Mockito.verify(orderService, times(1)).getOrderInformation(orderId, true)
    }
  }
  ...
  @Nested
  inner class GetRestrictedOrderDetails {
    @Test
    fun `Returns the appropriate object type`() {
      val orderId = "1ab"
      val expectedResult = OrderDetails(
      ...
      `when`(orderService.getOrderDetails(orderId, true)).thenReturn(expectedResult)

      val result = controller.getRestrictedOrderDetails(authentication, orderId)
    ...
    fun `gets order details from order service`() {
      ...
      Mockito.verify(orderService, times(1)).getOrderDetails(orderId, true)
    }
  }
