...
import uk.gov.justice.digital.hmpps.electronicmonitoringdatastoreapi.repositories.models.alcoholMonitoring.AthenaAlcoholMonitoringEquipmentDetailsDTO
import java.time.LocalDateTime

class AlcoholMonitoringEquipmentDetailsServiceTest {
  private lateinit var alcoholMonitoringEquipmentDetailsRepository: AlcoholMonitoringEquipmentDetailsRepository
  private lateinit var alcoholMonitoringEquipmentDetailsService: AlcoholMonitoringEquipmentDetailsService

  @BeforeEach
  fun setup() {
    alcoholMonitoringEquipmentDetailsRepository = Mockito.mock(AlcoholMonitoringEquipmentDetailsRepository::class.java)
    alcoholMonitoringEquipmentDetailsService = AlcoholMonitoringEquipmentDetailsService(alcoholMonitoringEquipmentDetailsRepository)
  }
  ...
  inner class GetEquipmentDetails {
    ...
    @BeforeEach
    fun setup() {
      Mockito.`when`(alcoholMonitoringEquipmentDetailsRepository.findByLegacySubjectId(legacySubjectId))
        .thenReturn(exampleEquipmentDetails)
    }

    @Test
    fun `calls getEquipmentDetails from amEquipmentDetailsRepository`() {
      alcoholMonitoringEquipmentDetailsService.getEquipmentDetails(legacySubjectId)

      Mockito.verify(alcoholMonitoringEquipmentDetailsRepository, Mockito.times(1)).findByLegacySubjectId(legacySubjectId)
    }

    @Test
    fun `returns correct details of the AlcoholMonitoringEquipmentDetails when a response is received`() {
      val result = alcoholMonitoringEquipmentDetailsService.getEquipmentDetails(legacySubjectId)
