...
class MonitoringEventsControllerTest {
  ...
  fun setup() {
    ...
    controller = MonitoringEventsController(orderEventsService, auditService)
  }
  ...
  inner class GetMonitoringEvents {
    ...
    fun `gets monitoring events from order events service`() {
      ...
      Mockito.`when`(orderEventsService.getMonitoringEvents(legacySubjectId)).thenReturn(expectedResult)
      ...
      Mockito.verify(orderEventsService, Mockito.times(1)).getMonitoringEvents(legacySubjectId)
    }
  }
}
