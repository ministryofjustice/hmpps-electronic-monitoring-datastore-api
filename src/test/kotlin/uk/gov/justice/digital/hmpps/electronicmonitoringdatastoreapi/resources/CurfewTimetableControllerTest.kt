...
class CurfewTimetableControllerTest {
  ...
  inner class GetCurfewTimetable {
    ...
    fun `gets order information from order service`() {
      ...
      Mockito.`when`(curfewTimetableService.getCurfewTimetable(orderId, false)).thenReturn(expectedResult)
      ...
      Mockito.verify(curfewTimetableService, Mockito.times(1)).getCurfewTimetable(orderId, false)
    }
  }
}
