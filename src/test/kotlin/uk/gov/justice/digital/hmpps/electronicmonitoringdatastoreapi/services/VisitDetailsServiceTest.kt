...
class VisitDetailsServiceTest {
  ...
  inner class GetVisitDetails {
    ...
    @BeforeEach
    fun setup() {
      Mockito.`when`(visitDetailsRepository.getVisitDetails(legacySubjectId, false))
    ...
    fun `calls getVisitDetails from order information repository`() {
      ...
      Mockito.verify(visitDetailsRepository, Mockito.times(1)).getVisitDetails(legacySubjectId, false)
    }

    @Test
    fun `returns a list of VisitDetails when a response is received`() {
      var result = service.getVisitDetails(legacySubjectId, false)
