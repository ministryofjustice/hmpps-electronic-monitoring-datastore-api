...
class ViolationEventsControllerTest {
  ...
  fun setup() {
    ...
    controller = ViolationEventsController(orderEventsService, auditService)
  }
  ...
  inner class GetViolationEvents {
    ...
    fun `gets violation events from order events service`() {
      ...
      Mockito.`when`(orderEventsService.getViolationEvents(legacySubjectId)).thenReturn(expectedResult)
      ...
      Mockito.verify(orderEventsService, Mockito.times(1)).getViolationEvents(legacySubjectId)
    }
  }
}
