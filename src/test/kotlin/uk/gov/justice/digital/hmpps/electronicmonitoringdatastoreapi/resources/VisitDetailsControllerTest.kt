...
class VisitDetailsControllerTest {
  ...
  inner class GetVisitDetails {
    ...
    fun `gets order information from order service`() {
      ...
      Mockito.`when`(visitDetailsService.getVisitDetails(orderId, false)).thenReturn(expectedResult)
      ...
      Mockito.verify(visitDetailsService, Mockito.times(1)).getVisitDetails(orderId, false)
    }
  }
}
