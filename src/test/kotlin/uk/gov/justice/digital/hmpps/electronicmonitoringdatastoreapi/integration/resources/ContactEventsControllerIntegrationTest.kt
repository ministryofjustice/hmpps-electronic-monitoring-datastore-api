...
class ContactEventsControllerIntegrationTest : ControllerIntegrationBase() {
  ...
  @Nested
  @DisplayName("GET /integrity/orders/{legacySubjectId}/contact-events")
  inner class GetContactEvents {
    ...
    fun `should return 401 unauthorized if no authorization header`() {
      noAuthHeaderRespondsWithUnauthorizedTest("/integrity/orders/234/contact-events")
    }

    @Test
    fun `should return 403 forbidden if no role in authorization header`() {
      noRoleInAuthHeaderRespondsWithForbiddenTest("/integrity/orders/234/contact-events")
    }

    @Test
    fun `should return 403 forbidden if wrong role in authorization header`() {
      wrongRolesRespondsWithForbiddenTest("/integrity/orders/234/contact-events", listOf("ROLE_WRONG"))
    }

    @Test
    fun `should throw a Bad Request exception if the URL param format is invalid`() {
      webTestClient.get()
        .uri("/integrity/orders/2_4/contact-events")
    ...
    fun `should return OK with valid auth header and role for accessing restricted orders`() {
      webTestClient.get()
        .uri("/integrity/orders/234/contact-events")
