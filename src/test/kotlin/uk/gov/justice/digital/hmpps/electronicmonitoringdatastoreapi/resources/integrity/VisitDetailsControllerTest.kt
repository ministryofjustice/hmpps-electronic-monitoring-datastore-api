...
class VisitDetailsControllerTest {
  ...
  fun setup() {
    ...
    controller = VisitDetailsController(visitDetailsService, auditService)
  }
  ...
  inner class GetVisitDetails {
    ...
    fun `gets order information from order service`() {
      ...
      Mockito.`when`(visitDetailsService.getVisitDetails(legacyOrderId)).thenReturn(expectedResult)
      ...
      Mockito.verify(visitDetailsService, Mockito.times(1)).getVisitDetails(legacyOrderId)
    }
  }
}
