...
class AthenaIntegritySuspensionOfVisitsQueryBuilderTest {

  fun replaceWhitespace(text: String): String = text.replace("\\s+".toRegex(), " ")

  val baseQuery: String = """
      SELECT 
        legacy_subject_id, suspension_of_visits, suspension_of_visits_end_date, suspension_of_visits_requested_date, suspension_of_visits_start_date, suspension_of_visits_start_time
      FROM 
  ...
  fun `returns valid SQL if legacySubjectId is populated`() {
    ...
    val result = AthenaIntegritySuspensionOfVisitsQueryBuilder()
  ...
  fun `throws an error if input contains SQL injection attack to show hidden records`() {
    ...
    Assertions.assertThatExceptionOfType(IllegalArgumentException::class.java).isThrownBy {
      AthenaIntegritySuspensionOfVisitsQueryBuilder()
