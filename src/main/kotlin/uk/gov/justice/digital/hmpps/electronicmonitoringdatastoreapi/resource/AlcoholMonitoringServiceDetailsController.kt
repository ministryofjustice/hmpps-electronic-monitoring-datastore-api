...
import uk.gov.justice.digital.hmpps.electronicmonitoringdatastoreapi.model.AlcoholMonitoringServiceDetails
import uk.gov.justice.digital.hmpps.electronicmonitoringdatastoreapi.service.AlcoholMonitoringServiceDetailsService
import uk.gov.justice.digital.hmpps.electronicmonitoringdatastoreapi.service.internal.AuditService
import uk.gov.justice.hmpps.kotlin.common.ErrorResponse

@Tag(name = TAG_ALCOHOL_ORDERS)
@RestController
class AlcoholMonitoringServiceDetailsController(
  @param:Autowired private val alcoholMonitoringServiceDetailsService: AlcoholMonitoringServiceDetailsService,
  @param:Autowired private val auditService: AuditService,
) {

  @Operation(
    summary = "Get the service details for an order",
    operationId = "getAlcoholMonitoringServiceDetails",
  )
  @RequestMapping(
    method = [RequestMethod.GET],
    path = [
      "/orders/alcohol-monitoring/{legacySubjectId}/service-details",
    ],
  ...
  ): ResponseEntity<List<AlcoholMonitoringServiceDetails>> {
    val result = alcoholMonitoringServiceDetailsService.getServiceDetails(legacySubjectId)

    auditService.createEvent(
      authentication.name,
      "GET_ALCOHOL_MONITORING_SERVICE_DETAILS",
      mapOf(
