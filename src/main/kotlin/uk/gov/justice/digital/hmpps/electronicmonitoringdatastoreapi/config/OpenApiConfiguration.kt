...
class OpenApiConfiguration(buildProperties: BuildProperties) {
  ...
  fun customOpenAPI(): OpenAPI = OpenAPI()
    .servers(
      listOf(
        Server().url("https://electronic-monitoring-datastore-api-dev.hmpps.service.justice.gov.uk")
          .description("Development"),
        Server().url("https://electronic-monitoring-datastore-api-preprod.hmpps.service.justice.gov.uk")
          .description("Pre-Production"),
        Server().url("https://electronic-monitoring-datastore-api.hmpps.service.justice.gov.uk")
    ...
    .tags(
      listOf(
        Tag().name(TAG_INTEGRITY_ORDERS).description("Provides access to Integrity monitoring order information."),
        Tag().name(TAG_ALCOHOL_ORDERS).description("Provides access to Alcohol monitoring order information."),
        Tag().name(TAG_CONNECTIVITY),
      ),
    )
    .info(
      Info().title("HMPPS Electronic Monitoring contract management information API").version(version)
        .contact(Contact().name("HMPPS Digital Studio").email("feedback@digital.justice.gov.uk"))
        .description(apiDescription())
        .license(
          License().name("The MIT License (MIT)").url("https://github.com/ministryofjustice/hmpps-electronic-monitoring-datastore-api/blob/main/LICENSE"),
        ),
    )
    .components(
      Components()
        .addSecuritySchemes(
          TOKEN_HMPPS_AUTH,
          SecurityScheme().addBearerJwtRequirement("`$ROLE_EM_DATASTORE_GENERAL__RO` or `$ROLE_EM_DATASTORE_RESTRICTED__RO`"),
        ),
    )
    .addSecurityItem(SecurityRequirement().addList(TOKEN_HMPPS_AUTH, listOf("read")))
}
