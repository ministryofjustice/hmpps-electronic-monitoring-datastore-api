...
class CurfewTimetableRepository(
  ...
  fun getCurfewTimetable(legacySubjectId: String, allowSpecials: Boolean): List<AthenaCurfewTimetableDTO> {
    val servicesQuery = ServicesQueryBuilder(athenaDatabase)
      .withLegacySubjectId(legacySubjectId)
      .build()

    val athenaResponse = athenaClient.getQueryResult(servicesQuery, allowSpecials)
